...
fun assert_owner_fails_for_non_owner() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);

    margin_position::assert_owner(&pos, @0xBAD);
...
fun request_close_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
...
#[test, expected_failure(abort_code = 2)] // EIntentPending
fun request_close_twice_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
...
fun cancel_intent_without_pending_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
    ...
    // confirm_open cleared the pending intent.
    let refund = margin_position::cancel_intent(&mut pos, TIMEOUT_MS, &clock, &mut ctx);
...
#[test, expected_failure(abort_code = 1)] // EWrongStatus
fun confirm_close_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
...
#[test, expected_failure(abort_code = 1)] // EWrongStatus
fun confirm_settle_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);

    margin_position::confirm_settle(&mut pos);
...
#[test]
fun confirm_settle_clears_open_position() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
    ...
    // Settlement clears the position regardless of any pending intent.
    margin_position::confirm_settle(&mut pos);
...
#[test, expected_failure(abort_code = 1)] // EWrongStatus
fun set_liquidation_flag_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
...
fun set_liquidation_flag_twice_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
...
#[test, expected_failure(abort_code = 1)] // EWrongStatus
fun apply_soft_liquidation_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);

    margin_position::apply_soft_liquidation(&mut pos, 1_000_000, 100_000);
...
#[test, expected_failure(abort_code = 1)] // EWrongStatus
fun apply_hard_liquidation_on_pending_open_fails() {
    let mut ctx = tx_context::dummy();
    let clock = clock::create_for_testing(&mut ctx);
    let mut pos = new_position(&mut ctx, &clock, 12_000, 1_000_000);
