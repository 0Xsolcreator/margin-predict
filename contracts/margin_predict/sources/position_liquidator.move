...
public fun flag_for_liquidation<T>(
    ...
    assert!(hf <= constants::HF_SOFT_BPS, ENotLiquidatable);
    let mode = if (hf <= constants::HF_HARD_BPS) { types::liq_hard() } else { types::liq_soft() };
...
///
/// `LIQ_REPORTER_BPS` (2 %) of proceeds are transferred to the flag's
/// reporter on-chain; the remainder is returned to the keeper.
public fun execute_liquidation<T>(
    ...
    let is_hard  = hf <= constants::HF_HARD_BPS;
    ...
    let reporter_cut = (((proceeds as u128) * (constants::LIQ_REPORTER_BPS as u128) / (constants::BPS as u128)) as u64);
    transfer::public_transfer(proceeds_coin.split(reporter_cut, ctx), reporter);

    if (full_close) {
        margin_position::apply_hard_liquidation(pos);
    } else {
        let new_debt = (((debt as u128) * ((constants::BPS - constants::SOFT_LIQ_FRACTION_BPS) as u128) / (constants::BPS as u128)) as u64);
...
fun compute_close_quantity(quantity: u64, is_hard: bool): u64 {
    ...
    let partial = (((quantity as u128) * (constants::SOFT_LIQ_FRACTION_BPS as u128) / (constants::BPS as u128)) as u64);
