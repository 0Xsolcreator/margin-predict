...
function LadderPanel({
  ...
  const currentP = +(
    min +
    Math.round((currentPrice - min) / step) * step
  ).toFixed(dec);
  ...
  const levels = useMemo(() => {
    ...
    for (let i = ROWS; i >= -ROWS; i--) {
      ...
      out.push({
        p,
        lr: prob ? (dir === 'up' ? (prob.up ?? 0) : (prob.down ?? 0)) : 0,
        hasData: !!prob,
        isCurrent: i === 0,
      });
    }
    return out;
  }, [currentP, step, dec, probabilities, dir]);
  ...
  const barTransition =
    everReadyRef.current && !isFirstReveal ? "width 0.5s ease" : "none";

  const currentRowRef = useRef(null);

  // Keep current price row centered whenever visible rows change
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 9, color: C.faint, letterSpacing: 2 }}>
          LEVELS
        </span>
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    height: 6,
                    borderRadius: 3,
                    opacity: 0.5 + 0.5 * Math.sin((i / (ROWS * 2)) * Math.PI),
                  }}
                />
              </div>
            ))
          : levels.filter(({ hasData, isCurrent }) => hasData || isCurrent).map(({ p, lr, hasData, isCurrent }) => {
                    ...
                    <>
                      ...
                      <div
                        style={{
                          ...
                          top: 0,
                          height: 1,
                          background: "rgba(212,245,107,0.4)",
                        }}
                      />
                      <div
                        style={{
                    ...
                    >
                      <div
                        style={{
                          ...
                          background:
                            "linear-gradient(to right,#f2785c,#d4f56b)",
                          borderRadius: 3,
