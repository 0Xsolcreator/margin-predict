...
vi.mock('../chain/contract.js', () => ({
  readOracleSettled: vi.fn(),
  readPositionFinancials: vi.fn(),
  buildSettlePosition: vi.fn(),
}));
...
describe('POST /positions/:positionId/settle', () => {
  it('400s when oracleId is missing', async () => {
    const app = buildApp({ logger: false });
    const res = await app.inject({
      method: 'POST',
      url: `/positions/${POSITION_ID}/settle`,
      payload: {},
    });
  ...
  it('409s when the oracle has not settled yet', async () => {
    vi.mocked(readOracleSettled).mockResolvedValue(false);
    const app = buildApp({ logger: false });
    const res = await app.inject({
      method: 'POST',
      url: `/positions/${POSITION_ID}/settle`,
    ...
    expect(res.json()).toEqual({ error: `Oracle ${ORACLE_ID} has not settled yet` });
    expect(readPositionFinancials).not.toHaveBeenCalled();
    expect(executeTransaction).not.toHaveBeenCalled();
  });

  it('settles a position end-to-end: redeem, unwind, repay, withdraw, untrack', async () => {
    const app = buildApp({ logger: false });
    const res = await app.inject({
      method: 'POST',
      url: `/positions/${POSITION_ID}/settle`,
    ...
    expect(readOracleSettled).toHaveBeenCalledWith(expect.anything(), KEEPER_ADDRESS, ORACLE_ID);
    expect(buildSettlePosition).toHaveBeenCalledWith(expect.anything(), POSITION_ID, ORACLE_ID);
    ...
    expect(executeTransaction).toHaveBeenCalledWith(marginClient, expect.anything(), expect.anything(), 'Settle position');
